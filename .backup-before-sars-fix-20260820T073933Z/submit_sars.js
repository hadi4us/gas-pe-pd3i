...
function _buildMasterIndex_() {
  ...
  const cFaskesKey = findCol_(["KodeFaskes", "Faskes Key", "KodeFaskes", "Kode Faskes", "FASKESKEY"]);
  const cNama      = findCol_(["NamaFaskes", "Nama Faskes", "NamaFasyankes", "Nama Fasyankes", "NAMA"]);
  const cPengampu  = findCol_(["Pengampu", "FaskesPengampu", "Faskes Pengampu", "PENGAMPU"]);
  ...
  for (let r = 1; r < values.length; r++) {
    ...
    // Index PKM/UPTD too: they are valid pengampu targets even when not
    // counted as reporting facilities in reminder/dashboard logic.
    const active = !status || ["AKTIF", "ACTIVE", "YA", "Y", "TRUE", "1", "WAJIB", "WAJIB LAPOR"].indexOf(status.toUpperCase()) !== -1;
    if (!active) continue;
    ...
    if (email && _isSarsReportingFacility_(typeKey, status)) byEmail[email] = entry;
  }
...
function submitSARS(formData) {
  ...
  const pengampuLookup = faskesPengampu ? _lookupFaskes_(masterIndex, faskesPengampu) : null;
  const faskesPengampuKey = _sTrim_(pengampuLookup && pengampuLookup.faskesKey);
  ...
  formData.cases.forEach((c) => {
    ...
    _setAny_(row, hmap, ["FaskesPengampuKey", "KodeFaskesPengampu", "Kode Faskes Pengampu"], faskesPengampuKey);
    _setAny_(row, hmap, ["FaskesKey", "KodeFaskes"], faskesKey);
