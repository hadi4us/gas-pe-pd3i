...
const ACCOUNT_REQUEST_HEADERS_ = Object.freeze([
  ...
  'faskes_key','Kecamatan','Kelurahan','Pengampu','KodePuskesmas','StatusPermohonan',
...
function _accountNormalizeRequest_(r,faskesMap){
  const code=_accountPick_(r,['faskes_key','KodeFaskes','KodePuskesmas','faskes_key']);
...
function submitAccountRequest(payload){
  try {
    ...
    if(!String(payload.namaFaskes||'').trim()||!String(payload.kodeFaskes||payload.kodePuskesmas||payload.faskes_key||'').trim())return _accountPublicError_('Pilih nama faskes dari master. Jangan isi manual di luar daftar.','VALIDATION');
    ...
    const lock=LockService.getScriptLock(); lock.waitLock(15000); try {
      ...
      const resolved=_accountResolveFaskes_(payload.kodeFaskes||payload.kodePuskesmas||payload.faskes_key||payload.namaFaskes||payload.unitKerja), faskesName=String(payload.namaFaskes||resolved.nama||payload.unitKerja||'').trim(), faskesCode=String(payload.kodeFaskes||payload.kodePuskesmas||payload.faskes_key||resolved.kode||'').trim();
      const id=_accountRequestId_(), h=_accountHeaders_(sh), row=h.map(function(x){return ({RequestId:id,SubmittedAt:new Date(),nama_petugas:nama,email:email,NoWhatsapp:noWhatsapp,jenis:String(payload.jenisUnit||''),nama_faskes:faskesName,AlamatFaskes:String(payload.alamatFaskes||payload.alamatUsulan||((payload.master&&payload.master.alamat)||'')).trim(),UnitKerja:String(payload.unitKerja||faskesName),faskes_key:faskesCode,Kecamatan:String(payload.kecamatan||''),Kelurahan:String(payload.kelurahan||''),Pengampu:String(payload.pengampu||''),KodePuskesmas:faskesCode,StatusPermohonan:ACCOUNT_REQUEST_STATUS_.PENDING,ConsentAccepted:true,TelegramChatId:'',TelegramUsername:''}[x]||'');});
