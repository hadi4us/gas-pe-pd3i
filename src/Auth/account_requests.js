...
function _accountNormalizeRequest_(r,faskesMap){
  const code=_accountPick_(r,['KodeFaskes','KodePuskesmas','FaskesKey']);
  const name=_accountPick_(r,['NamaFaskes']);
  const master=(faskesMap&&faskesMap[_accountNormalize_(code)])||(faskesMap&&faskesMap[_accountNormalize_(name)])||{};
  if(!String(r.NamaFaskes||'').trim())r.NamaFaskes=master.nama||'';
  if(!String(r.KodeFaskes||'').trim())r.KodeFaskes=master.kode||code;
  if(!String(r.AlamatFaskes||'').trim())r.AlamatFaskes=_accountPick_(r,['Alamat Faskes'])||master.alamat||'';
  if(!String(r['Alamat Faskes']||'').trim())r['Alamat Faskes']=r.AlamatFaskes||'';
...
function _accountFindUserEmail_(email){ const sh=getSheetOrNull_('REF_USER'); if(!sh)return false; const h=_accountHeaders_(sh), ix=h.indexOf('Gmail')>=0?h.indexOf('Gmail'):h.indexOf('Email'); if(ix<0)return false; return sh.getDataRange().getDisplayValues().slice(1).some(function(r){return _accountEmail_(r[ix])===email;}); }
function _accountPendingEmail_(sh,email){ const h=_accountHeaders_(sh), vals=sh.getDataRange().getDisplayValues(); const ie=h.indexOf('Gmail'), is=h.indexOf('StatusPermohonan'); return vals.slice(1).some(function(r){return ie>=0&&is>=0&&_accountEmail_(r[ie])===email&&String(r[is]).trim()===ACCOUNT_REQUEST_STATUS_.PENDING;}); }
...
function requestTelegramPairingForAccountRequest(requestId, email){
  try {
    ...
    const ir=h.indexOf('RequestId'), ie=h.indexOf('Gmail'), is=h.indexOf('StatusPermohonan');
