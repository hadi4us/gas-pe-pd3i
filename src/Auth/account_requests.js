...
function _accountPendingEmail_(sh,email){ const h=_accountHeaders_(sh), vals=sh.getDataRange().getDisplayValues(); const ie=_accountIndex_(h,['Gmail','Email']), is=_accountIndex_(h,['StatusPermohonan']); return vals.slice(1).some(function(r){return ie>=0&&is>=0&&_accountEmail_(r[ie])===email&&String(r[is]).trim()===ACCOUNT_REQUEST_STATUS_.PENDING;}); }
...
function requestTelegramPairingForAccountRequest(requestId, email){
  try {
    ...
    const ir=_accountIndex_(h,['RequestId']), ie=_accountIndex_(h,['Gmail','Email']), is=_accountIndex_(h,['StatusPermohonan']);
